...
# Parameters
learning_rate = 0.001
training_iters = 50                #外循环次数
batch_size = 500
minibatch_size = 100                #批量梯度下降批大小
repeat_round = 1                  #shuffle 次数
...
result_path = "F:\\data\\tianchi\\CIKM2017\\CIKM2017_train\\data_new\\CIKM2017_train\\rnn_results\\rnn_result{}.csv"
model_path = "F:\\data\\tianchi\\CIKM2017\\CIKM2017_train\\data_new\\CIKM2017_train\\rnn_models\\rnn_model{}.ckpt"

save_train_path = "F:\\data\\tianchi\\CIKM2017\\CIKM2017_train\\data_new\\CIKM2017_train\\new_features\\feature_rnn_train.npy"
save_test_path =  "F:\\data\\tianchi\\CIKM2017\\CIKM2017_train\\data_new\\CIKM2017_train\\new_features\\feature_rnn_test.npy"
#linux
...
train = []
for k,batch_train in train_iterator:
    if k == 2:
        break
    train.append(feature_change(batch_train))
train = np.array(train).reshpae(10000,15,-1)
print(train.shape)
...
with tf.Session() as sess:
    ...
    for i in range(training_iters):
        ...
        for m ,(minibatch_test, test_lables) in enumerate(test_iterator):
            ...
            minibatch_test = minibatch_test.reshape((batch_size, n_steps,n_input))
            temp = sess.run(pred,feed_dict={x: minibatch_test})
            result.append(temp)
